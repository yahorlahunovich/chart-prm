"""
Custom KTO Trainer engine for Qwen2.5-VL.
"""

from typing import Any, Callable, Dict, List, Optional, Tuple

import torch

from chart_prm.kto.loss import kto_loss
from chart_prm.kto.utils import build_qwen_kto_batch


def compute_sequence_logprobs(
    model: Any,
    batch: Dict[str, torch.Tensor],
    is_reference: bool = False,
) -> torch.Tensor:
    """
    Computes sequence log-probabilities for a given model and batch.

    Args:
        model: Trainable policy model (or base model with disabled adapters)
        batch: Formatted Qwen2.5-VL batch containing input_ids, labels, attention_mask, etc.
        is_reference: If True, evaluates in reference mode (using model.disable_adapter() if PEFT model)

    Returns:
        seq_logprobs: (batch_size,) tensor of sequence log-probabilities
    """
    from chart_prm.dpo.utils import sequence_logprob

    # Exclude labels and kto_labels from model forward arguments
    model_inputs = {k: v for k, v in batch.items() if k not in ("labels", "kto_labels")}

    if is_reference and hasattr(model, "disable_adapter"):
        with model.disable_adapter():
            outputs = model(**model_inputs)
    else:
        outputs = model(**model_inputs)

    logits = outputs.logits if hasattr(outputs, "logits") else outputs
    labels = batch["labels"]

    seq_logps = sequence_logprob(logits, labels)
    return seq_logps


def train_kto_step(
    model: Any,
    optimizer: torch.optim.Optimizer,
    batch: Dict[str, torch.Tensor],
    ref_model: Optional[Any] = None,
    beta: float = 0.1,
    desirable_weight: float = 1.0,
    undesirable_weight: float = 1.0,
    max_grad_norm: Optional[float] = 1.0,
) -> Tuple[torch.Tensor, Dict[str, float]]:
    """
    Executes a single minimal KTO training step with VRAM cache clearing.

    Args:
        model: Qwen2.5-VL policy model
        optimizer: PyTorch optimizer
        batch: Formatted KTO batch dict (contains input_ids, labels, kto_labels, attention_mask, etc.)
        ref_model: Optional frozen reference model
        beta: KTO beta parameter
        desirable_weight: Multiplier weight for desirable completions
        undesirable_weight: Multiplier weight for undesirable completions
        max_grad_norm: Maximum gradient norm for clipping

    Returns:
        loss: Scalar loss tensor
        metrics: Dictionary of metric values
    """
    model.train()

    # Policy model sequence log-probabilities
    policy_logp = compute_sequence_logprobs(model, batch, is_reference=False)
    if torch.cuda.is_available():
        torch.cuda.empty_cache()

    # Frozen reference model sequence log-probabilities
    if ref_model is not None:
        ref_model.eval()
        with torch.no_grad():
            ref_logp = compute_sequence_logprobs(ref_model, batch, is_reference=False)
    else:
        with torch.no_grad():
            ref_logp = compute_sequence_logprobs(model, batch, is_reference=True)

    if torch.cuda.is_available():
        torch.cuda.empty_cache()

    kto_labels = batch["kto_labels"]

    # Compute KTO loss
    loss, metrics = kto_loss(
        policy_logp=policy_logp,
        ref_logp=ref_logp,
        kto_labels=kto_labels,
        beta=beta,
        desirable_weight=desirable_weight,
        undesirable_weight=undesirable_weight,
    )

    optimizer.zero_grad()
    loss.backward()

    if max_grad_norm is not None:
        trainable_params = [p for p in model.parameters() if p.requires_grad]
        if trainable_params:
            torch.nn.utils.clip_grad_norm_(trainable_params, max_norm=max_grad_norm)

    optimizer.step()

    if torch.cuda.is_available():
        torch.cuda.empty_cache()

    return loss, metrics


def fit_kto(
    model: Any,
    dataset: List[Dict[str, Any]],
    processor: Any,
    ref_model: Optional[Any] = None,
    lr: float = 1e-5,
    beta: float = 0.1,
    desirable_weight: float = 1.0,
    undesirable_weight: float = 1.0,
    epochs: int = 1,
    batch_size: int = 1,
    device: Optional[torch.device] = None,
    on_step_end: Optional[Callable[[int, Dict[str, float]], None]] = None,
) -> List[Dict[str, float]]:
    """
    Fits policy model on dataset using KTO (Kahneman-Tversky Optimization).

    Args:
        model: Qwen2.5-VL policy model
        dataset: KTO dataset items
        processor: Qwen2.5-VL processor
        ref_model: Optional frozen reference model
        lr: Learning rate
        beta: KTO beta parameter
        desirable_weight: Multiplier weight for desirable completions
        undesirable_weight: Multiplier weight for undesirable completions
        epochs: Number of training epochs
        batch_size: Batch size per step
        device: Torch compute device
        on_step_end: Optional callback executed at step end

    Returns:
        history: List of metric dicts for each step
    """
    trainable_params = [p for p in model.parameters() if p.requires_grad]
    if not trainable_params:
        raise ValueError("Model has no trainable parameters! Ensure LoRA adapters are attached.")

    optimizer = torch.optim.AdamW(trainable_params, lr=lr)

    history = []
    global_step = 0

    for epoch in range(epochs):
        print(f"--- Epoch {epoch + 1}/{epochs} ---")
        for i in range(0, len(dataset), batch_size):
            batch_items = dataset[i : i + batch_size]

            batch = build_qwen_kto_batch(
                processor=processor,
                items=batch_items,
                device=device,
            )

            loss, metrics = train_kto_step(
                model=model,
                optimizer=optimizer,
                batch=batch,
                ref_model=ref_model,
                beta=beta,
                desirable_weight=desirable_weight,
                undesirable_weight=undesirable_weight,
            )

            global_step += 1
            step_metrics = {"step": global_step, "epoch": epoch + 1, **metrics}
            history.append(step_metrics)

            if on_step_end is not None:
                on_step_end(global_step, metrics)

    return history
