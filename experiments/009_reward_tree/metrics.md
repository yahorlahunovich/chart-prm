...
- **Update — now v1.** `scripts/evaluation/distill_reward_tree_criteria.py` ran the LLM-distillation pass this write-up originally deferred: one Gemini call per parent (9 calls, batching all its children, no image needed) turns each child's TF-IDF terms + one exemplar sentence into a clean, general rubric statement, e.g. `axis_or_layout_misread_1`: *"Accurately identify data point coordinates and peak locations by directly aligning curves with their corresponding axis gridlines."* All 33/33 children distilled successfully; `chart_prm.dynamic_scoring.format_criterion` prefers `rubric_text` when present and falls back to the v0 keywords/exemplar phrasing for any child that isn't (none currently). The tree's `child_criteria_status` field records this.
- **No new judge/API calls were made to build the tree itself** (children, merge threshold, tree structure) — all of that is recomputed from data already in the repo (`fail_analyses_categorized.csv` + `fail_analysis_embeddings.npy`). The distillation pass above is the one part of this experiment that does call the judge, and it's text-only and cheap (9 calls).
