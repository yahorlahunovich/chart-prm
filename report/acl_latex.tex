...
Interpreting scientific literature requires multimodal models to reason over complex visual data, including multi-panel plots, heterogeneous coordinate frames, and dense data distributions. While frontier Multimodal Large Language Models (MLLMs) demonstrate impressive visual fluency, smaller and computationally efficient models ($<7\text{B}$ parameters) frequently fail when tasked with multi-step reasoning over scientific charts. Standard post-training alignment and evaluation protocols predominantly rely on Outcome Reward Models (ORMs) or sparse sequence-level feedback \citep{ouyang2022instructgpt}, penalizing or rewarding models based strictly on the correctness of their final answer. In complex reasoning chains, however, sparse outcome supervision suffers from severe credit assignment ambiguity: it cannot distinguish an early perceptual misreading from a late arithmetic error, frequently penalizing sound deductive logic that followed a minor visual slip or inadvertently reinforcing flawed reasoning that stumbled upon the correct answer by chance.

To resolve this credit assignment bottleneck, Process Reward Models (PRMs) decompose long trajectories into discrete, verifiable reasoning steps \citep{lightman2023prm}. While process supervision has achieved considerable success in mathematical reasoning, its dynamics in multimodal vision-language tasks remain under-explored. Chart reasoning introduces a unique operational dichotomy: unlike purely textual or symbolic domains, chart question answering requires models to continually interleave fine-grained visual grounding (such as resolving tick marks, deciphering dual axes, and linking colored markers to legend keys) with symbolic deduction (such as computing rate changes or identifying extrema). 

In this work, we address a foundational research hypothesis: \textit{Can a compact vision-language model reason effectively over complex scientific charts if it can be aligned using only a small, process-supervised dataset?} Frontier model alignment routinely depends on tens of thousands of costly, human-annotated trajectories. We investigate whether fine-grained process verification can overcome data scarcity, enabling a lightweight vision-language model to achieve meaningful reasoning gains from fewer than 150 high-quality contrastive preference pairs under accessible single-GPU compute constraints.

We investigate this hypothesis using the rigorous \textbf{CharXiv} benchmark \citep{wang2024charxiv}, which curates real-world scientific figures published in arXiv papers across eight academic disciplines (Computer Science, Physics, Mathematics, Economics, Quantitative Biology, Quantitative Finance, Statistics, and Electrical Engineering). While CharXiv includes surface-level \textit{descriptive} queries (simple OCR lookups), we isolate a balanced, domain-stratified subset of \textbf{500 reasoning questions} across 62 distinct chart types, specifically targeting queries that necessitate multi-step perceptual and cognitive synthesis. Operating strictly within consumer-grade hardware limits (a single 16GB Nvidia Tesla T4 GPU), we benchmark the compact \texttt{Qwen2.5-VL-3B-Instruct} model \citep{bai2025qwen25vl} using 4-bit NF4 quantization \citep{dettmers2023qlora} and Low-Rank Adaptation (LoRA) \citep{hu2021lora}.

Using an advanced multimodal LLM-as-a-judge (\texttt{muse-spark-1.1}) \citep{meta2026musespark, zheng2023judging}, we evaluate 1,274 multi-step rollouts comprising 4,947 individual reasoning steps. We develop a rollout-level batching protocol to make step-by-step verification computationally tractable and token-efficient. Textual and statistical analysis of 2,920 refuted reasoning steps reveals two critical diagnostic insights:
\begin{enumerate}[noitemsep,topsep=0pt]
    \item \textbf{The Perceptual Bottleneck}: Model failures are overwhelmingly visual rather than logical. Axis and layout misreadings (24.0\%) combined with series and legend color confusion (19.5\%) account for \textbf{43.5\%} of all failed steps, whereas pure arithmetic calculation mistakes account for only \textbf{1.3\%} (1 in 77 errors). The compact model possesses capable symbolic logic, but its visual grounding routinely misreads the quantitative premises.
    \item \textbf{Catastrophic Error Cascading}: Mistakes occur early in the trajectory: \textbf{79.7\%} of all erroneous rollouts make their first mistake in the opening two steps (Steps 0--1). Once an error occurs at step $N$, downstream steps fail \textbf{82.7\%} of the time ($P(S_{N+1}=0 \mid S_N=0) = 82.7\%$), confirming that uncorrected visual misreadings act as poisoned premises for all subsequent deductions.
\end{enumerate}

Leveraging these step-level diagnostics, we curate small preference datasets and benchmark five post-training alignment paradigms: Supervised Fine-Tuning (SFT, 70 traces), Full-Sequence Direct Preference Optimization (DPO, 134 pairs) \citep{rafailov2023dpo}, Suffix Step-DPO (54 pairs) \citep{lai2024stepdpo}, Kahneman-Tversky Optimization (KTO, 84 positive / 252 negative rollouts) \citep{ethayarajh2024kto}, and sequential two-stage SFT$\rightarrow$DPO. Evaluated on a held-out test split of 100 CharXiv reasoning problems, our experiments provide strong empirical evidence for our small-data hypothesis: Full DPO trained on only 134 contrastive pairs achieves the highest exact-match accuracy of \textbf{29.0\%} (+3.0 percentage points over the base model). In contrast, SFT on 70 verified traces enforces 100\% formatting compliance but suffers from rigid template memorization, degrading reasoning accuracy to 23.0\%. Furthermore, unpaired KTO achieves 66.0\% latent ground-truth recall in prose but exhibits structural formatting collapse.

Our main contributions are summarized as follows:
\begin{itemize}[noitemsep,topsep=0pt]
    \item We formulate and empirically validate the small-data alignment hypothesis for multimodal chart reasoning, demonstrating that 134 process-curated preference pairs improve the exact-match accuracy of \texttt{Qwen2.5-VL-3B-Instruct} under a single-T4 GPU budget.
    \item We construct an empirical 9-category error taxonomy across 2,920 refuted reasoning steps on CharXiv, establishing that visual grounding failures (43.5\%) vastly outweigh intermediate arithmetic mistakes (1.3\%).
    \item We quantify step-level error progression dynamics, showing that $\sim$80\% of initial mistakes occur within the first two steps and propagate downstream with an 82.7\% cascade probability.
    \item We benchmark five alignment strategies and characterize a fundamental Pareto frontier between structural instruction compliance and latent reasoning recall in preference-tuned vision-language models.
\end{itemize}
