...
In this paper, we investigate a central hypothesis: \textit{Can a compact vision-language model reason effectively over complex scientific charts when aligned with only a small, process-supervised dataset?} Rather than relying on tens of thousands of annotations, we study whether fewer than 150 verified contrastive preference pairs suffice to align a compact model under consumer-grade compute constraints.

We evaluate this hypothesis on the \textbf{CharXiv} benchmark \citep{wang2024charxiv}. Discarding surface-level descriptive queries, we isolate a stratified subset of \textbf{500 multi-step reasoning questions} spanning 8 academic domains and 62 chart types. Operating within a single 16GB Nvidia T4 GPU budget, we benchmark \texttt{Qwen2.5-VL-3B-Instruct} \citep{bai2025qwen25vl} using 4-bit NF4 quantization \citep{dettmers2023qlora} and LoRA \citep{hu2021lora}.
...
Leveraging these diagnostics, we benchmark five alignment strategies on a 100-question held-out set: SFT (70 traces), Full DPO (134 pairs) \citep{rafailov2023dpo}, Suffix Step-DPO (54 pairs) \citep{lai2024stepdpo}, KTO (84 pos / 252 neg) \citep{ethayarajh2024kto}, and SFT$\rightarrow$DPO. Confirming our small-data hypothesis, Full DPO on just 134 pairs achieves the highest accuracy at 29.0\% (+3.0 pp over the 26.0\% base). SFT enforces 100\% formatting compliance but lowers accuracy to 23.0\% via template rigidity, while KTO attains 66.0\% latent ground-truth recall in text but suffers structural format collapse.
...
\begin{itemize}[noitemsep,topsep=0pt]
    ...
    \item Benchmarking five alignment methods and mapping the Pareto frontier between structural compliance and latent reasoning recall.
\end{itemize}
