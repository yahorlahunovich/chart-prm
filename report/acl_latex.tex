...
To understand why the model fails, we analyze the natural language critique texts written by the judge across all 2,920 failed steps ($s_t = 0$). We categorize these critiques into a 9-category taxonomy using keyword matching and semantic clustering (see Table~\ref{tab:appendix_error_taxonomy} in Appendix~\ref{sec:appendix_diagnostics} for diagnostic keywords and critique excerpts). Figure~\ref{fig:taxonomy_dist} shows the overall distribution, while Figure~\ref{fig:error_by_step_depth} details how error modes evolve across step depth.
