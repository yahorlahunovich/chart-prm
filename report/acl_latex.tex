...
\textbf{1. Supervised Fine-Tuning (SFT):} SFT maximizes the log-likelihood of verified gold reasoning steps:
\begin{equation}
...
\subsection{Hardware-Constrained Implementation on Single T4 GPU}
...
A key design requirement of our work is reproducibility under realistic consumer compute constraints. All model training is conducted on a single 16GB Nvidia T4 GPU (standard Google Colab tier).

To train a 3B vision-language model on a single GPU without out-of-memory errors, we employ three memory optimizations:
\begin{itemize}[noitemsep,topsep=0pt]
    \item \textbf{4-Bit NormalFloat Quantization (NF4):} We load the base vision-language backbone in 4-bit precision with double quantization via \texttt{bitsandbytes} \citep{dettmers2023qlora}.
    \item \textbf{Low-Rank Adaptation (LoRA):} We freeze the base model weights and attach LoRA adapters \citep{hu2021lora} to all linear projection layers in both the language model and attention modules (\texttt{q\_proj}, \texttt{k\_proj}, \texttt{v\_proj}, \texttt{o\_proj}, \texttt{gate\_proj}, \texttt{up\_proj}, \texttt{down\_proj}). We set rank $r=16$, scaling factor $\alpha=32$, and dropout to $0.05$.
    \item \textbf{Gradient Checkpointing:} We enable gradient checkpointing and use a per-device batch size of 1 with gradient accumulation steps (effective batch size 8).
\end{itemize}

Table~\ref{tab:training_setup} reports the training configurations, dataset sizes, learning rates, and wall-clock times across all five methods. Memory usage stays strictly below 14.2\,GB throughout training. SFT converges in 12 minutes, and Full DPO completes in 22 minutes. This confirms that process-level multimodal alignment can be performed efficiently without expensive multi-node GPU clusters.
