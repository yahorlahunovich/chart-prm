#!/usr/bin/env python3
"""
Minimal Custom Supervised Fine-Tuning (SFT) Training Script for Qwen2.5-VL.

Usage:
  python scripts/train/train_sft.py --smoke-only
  python scripts/train/train_sft.py --dataset-path experiments/001_500_reasoning/data/sft_samples.jsonl --epochs 3
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Minimal Custom SFT Trainer for Qwen2.5-VL")
    parser.add_argument(
        "--dataset-path",
        type=str,
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="adapters/qwen_vl_sft_adapter",
        help="Directory to save final trained SFT adapter/model",
    )
...
def main():
    ...
    def print_step_log(step, metrics):
        print(
            f"[Step {step:03d}] SFT Loss: {metrics['loss']:.4f} | "
            f"Tokens: {metrics['valid_tokens']:.0f}"
        )
    ...
    print(f"Saving trained SFT model and processor to {out_dir}...")
