...
Usage:
  python scripts/train/train_dpo.py --smoke-only
  python scripts/train/train_dpo.py --dataset-path experiments/001_500_reasoning/data/dpo_pairs.jsonl --epochs 2
"""
...
def parse_args():
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="adapters/qwen_vl_dpo_adapter",
...
def _apply_mode_defaults(args: argparse.Namespace) -> argparse.Namespace:
    ...
    default_output = "adapters/qwen_vl_dpo_adapter"
    ...
    if args.step_dpo:
        ...
        if args.output_dir == default_output:
            args.output_dir = "adapters/qwen_vl_step_dpo_adapter"
        # Suffix-from-divergence pairs include Final Answer; keep the fragment guard on.
    if args.sft_dpo:
        if args.output_dir == default_output:
            args.output_dir = "adapters/qwen_vl_sft_dpo_adapter"
        if args.lr == 1e-5:
            args.lr = 2e-6
        if args.epochs == 2:
            args.epochs = 1
