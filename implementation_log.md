...
- **Fix**: First Kaggle run failed on `ImportError: TransformGetItemToIndex` from a mismatched default torch/transformers stack. The install cell now always force-uninstalls and pins `torch==2.5.1+cu124` with `transformers==4.49.0` / `peft==0.14.0` (same stack as the successful training kernels), not only on P100.
